...
from config import BATCH_SIZE, CLASS_NAMES_PATH, DATASET_NAME, MODEL_PATH, NUM_WORKERS, SEED
...
def main():
    ...
    with open(CLASS_NAMES_PATH, "w", encoding="utf-8") as file:
        file.write("\n".join(class_names))
    _, _, test_rows = split_rows(dataset, SEED)
    loader = DataLoader(WasteDataset(test_rows), batch_size=BATCH_SIZE, shuffle=False, num_workers=NUM_WORKERS, pin_memory=torch.cuda.is_available())
    ...
    with open("outputs/classification_report.txt", "w", encoding="utf-8") as file:
        file.write(f"Test samples: {len(y_true)}\nAccuracy: {accuracy:.4f}\nWeighted Precision: {precision:.4f}\nWeighted Recall: {recall:.4f}\nWeighted F1: {f1:.4f}\n\n{report}")
