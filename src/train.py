...
def build_class_weights(train_rows, num_classes, device):
    counts = Counter(label for _, label in train_rows)
    total = sum(counts.values())

    # Square-root inverse frequency is less aggressive than 1/frequency
    # while still reducing the effect of class imbalance.
    weights = []
...
def main():
    ...
    print("Preparing expanded 14-class waste dataset...")
    ...
    print(
        f"Split sizes: train={len(train_rows)}, "
    ...
    class_weights, class_counts = build_class_weights(
        train_rows, len(class_names), device
    )
    print("Training class counts:")
    for label, name in enumerate(class_names):
        print(f"  {name}: {class_counts[label]}")
    ...
    for epoch in range(EPOCHS):
        train_loss, train_acc = run_epoch(
            model, train_loader, loss_fn, optimizer, device, True
        )
        val_loss, val_acc = run_epoch(
            model, val_loader, loss_fn, optimizer, device, False
        )
    ...
    print("Training complete. Run src/evaluate.py for final 14-class test metrics.")
